...
import { 
  ...
  TrendingUp, BarChart3, ChevronDown, UserPlus, Shield
} from 'lucide-react';
...
const AdminTeams = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setData({
        ...
        leaders: usersRes.data.data.filter(u => u.role === 'team_leader' || u.role === 'project_manager' || u.role === 'admin'),
  ...
  if (data.loading) return (
    <div className="page-wrapper" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="pulse-animation" style={{ fontWeight: 800, color: 'var(--primary)' }}>INITIALIZING TEAM RADAR...</div>
    </div>
  );
    ...
    return (
      <div className="page-wrapper fade-in" style={{ padding: '2rem 3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3.5rem' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', fontWeight: 950, letterSpacing: '-1.5px' }}>Team Construction</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', marginTop: '0.5rem' }}>Structuring personnel for strategic project execution.</p>
          </div>
          <button className="btn glass" onClick={() => setView('list')} style={{ borderRadius: '14px', fontWeight: 800 }}>Back to Fleet</button>
        </div>
          ...
          <div style={{ maxWidth: '850px' }}>
            <div className="card" style={{ padding: '3rem', borderRadius: '32px', marginBottom: '3rem' }}>
              <div className="form-group" style={{ marginBottom: '2.5rem' }}>
                <label className="form-label" style={{ fontWeight: 800, fontSize: '0.8rem' }}>TEAM DESIGNATION</label>
                <input 
    ...
    <div className="page-wrapper fade-in" style={{ padding: '2rem 2.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', marginTop: '0.5rem' }}>Managing <span style={{ color: 'var(--primary)', fontWeight: 800 }}>{data.teams.length} specialized units</span>.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
         <div className="card" style={{ padding: '2rem', borderRadius: '24px', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <div>
               <div style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Total Personnel</div>
         ...
         <div className="card" style={{ padding: '2rem', borderRadius: '24px', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <div>
               <div style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Specialized Units</div>
         ...
         <div className="card" style={{ padding: '2rem', borderRadius: '24px', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <div>
               <div style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Capacity Index</div>
               <div style={{ fontSize: '1.75rem', fontWeight: 900 }}>88%</div>
            </div>
         </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ background: '#f8fafc', padding: '1.25rem', borderRadius: '20px', marginBottom: '1.5rem' }}>
                 <div style={{ fontSize: '0.75rem', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>TEAM LEADER</div>
                 <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                       <div style={{ fontSize: '1.1rem', fontWeight: 800 }}>{team.team_leader_name}</div>
                    </div>
                 </div>
              </div>

              <div style={{ marginBottom: '2rem' }}>
                 <div style={{ fontSize: '0.75rem', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '1rem', letterSpacing: '0.05em' }}>TEAM MEMBERS ({team.member_count})</div>
                 <div style={{ fontSize: '0.95rem', color: 'var(--text-main)', fontWeight: 600, lineHeight: 1.5, minHeight: '3em' }}>
                    {team.member_names || 'Zero personnel assigned'}
                 </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', borderTop: '1px solid #f1f5f9', paddingTop: '1.5rem' }}>
                 <Badge status={workload.level}>{workload.label.toUpperCase()}</Badge>
                 <button 
                  className="btn glass" 
                  style={{ borderRadius: '12px', fontSize: '0.85rem', fontWeight: 800 }}
          ...
          <div style={{ 
            ...
            >
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '2rem', lineHeight: 1.5 }}>
                 This action is permanent and cannot be undone. All team data and memberships will be lost.
              </p>
      ...
      <ManageMembersModal 
        ...
        onClose={() => { setShowMemberModal(false); fetchData(); }}
        team={selectedTeam}
        allUsers={data.users}
      />
    </div>
...
const ManageMembersModal = ({ isOpen, onClose, team, allUsers }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: '1rem' }}>
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="card" style={{ maxWidth: '750px', width: '100%', borderRadius: '32px', overflow: 'hidden', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
